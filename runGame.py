...
def runGame(ccgame, agents: dict):
    ...
    while (not ccgame.isEnd(state, itert)) and itert < max_iter:
        itert += 1
        board.board = state[1]
        board.draw()
    ...
    if ccgame.isEnd(state, itert):
        return state[1].isEnd(itert)[1]  # return winner
...
def callback(ccgame):
    ...
    simulateMultipleGames({1: simpleGreedyAgent1, 2: teamAgent}, 20, ccgame)
    # simulateMultipleGames({1: teamAgent, 2: simpleGreedyAgent}, 20, ccgame)
